...
### 4. プロフィール画像の枠線（ボーダー）削除
- `index.html` のプロフィール画像を表示している枠組み（`.about-image-wrap`）の境界に引かれていた `1px` の境界線（`border: 1px solid var(--border);`）を削除しました。これにより、枠線のないシームレスでフラットなプロフィール画像の表示になりました。
